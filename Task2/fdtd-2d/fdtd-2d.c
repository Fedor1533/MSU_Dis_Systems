...
static
void init_array (int tmax, int nx, int ny, double ex[ nx][ny], double ey[ nx][ny], double hz[ nx][ny], double _fict_[ tmax])
{
  int i, j;
...
static void kernel_fdtd_2d(int tmax, int nx, int ny, double ex[ nx][ny], double ey[ nx][ny], double hz[ nx][ny], double _fict_[ tmax])
{
  int t, i, j;
  ...
  for(t = 0; t < tmax; t++)
    {
      for (j = 0; j < ny; j++)
        ey[0][j] = _fict_[t]; //for 0 process

      for (i = 1; i < nx; i++)
        for (j = 0; j < ny; j++)
            ey[i][j] = ey[i][j] - 0.5*(hz[i][j]-hz[i-1][j]); //need previous row
      ...
      for (i = 0; i < nx - 1; i++)
        for (j = 0; j < ny - 1; j++)
             hz[i][j] = hz[i][j] - 0.7* (ex[i][j+1] - ex[i][j] + ey[i+1][j] - ey[i][j]); //need next row
    }
}
